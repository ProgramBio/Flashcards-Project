...
BG_EDIT_DECK_3 = load_image_asset("Image/Background/deck_editing_room1.png", (screen_width, screen_height))
BG_PREPARE_DECK = load_image_asset("Image/Background/deck_editing_room.png", (screen_width, screen_height))
...
def load_animation_frames(sheet_path, frame_count, frame_width, frame_height, scale_to_size=None):
    """โหลดแอนิเมชั่นจาก sprite sheet"""

    frames = []
    try:
        sheet = pygame.image.load(sheet_path).convert_alpha()
    except Exception as e:
        print(f"!!! Animation Error: Failed to load sheet '{sheet_path}'.{e}")

        fallback = pygame.Surface(scale_to_size if scale_to_size else (frame_width, frame_height))
        fallback.fill((255, 0, 255))
        return [fallback]

    for i in range(frame_count):
        frame_rect = pygame.Rect(i * frame_width, 0, frame_width, frame_height)
        frame_img = pygame.Surface(frame_rect.size, pygame.SRCALPHA)
        frame_img.blit(sheet, (0, 0), frame_rect)

        if scale_to_size:
            frame_img = pygame.transform.scale(frame_img, scale_to_size)
...
def battle_screen(deck_name, count_cards, is_boss_stage):
    ...
    try:
        ...
        # --- Witch Animation ---
        witch_frames_right = load_animation_frames("Image/MC_spritesheet.png", 12, 68, 87, (272, 348))

        witch_frames_left = [pygame.transform.flip(frame, True, False) for frame in witch_frames_right]

        witch_anim_index = 0
        witch_anim_last_update = pygame.time.get_ticks()
        witch_anim_speed = 100

        red_overlay_witch = pygame.Surface((272, 348), pygame.SRCALPHA)
    ...
    def handle_answer(answer_type):
        ...
        if answer_type == "ANSWER_KNOW":
            ...
            witch_center_x = (screen_width//2 - 710) + (348 // 2)
    ...
    while running:
        ...
        now = pygame.time.get_ticks() #Witch
        if now - witch_anim_last_update > witch_anim_speed:
            witch_anim_last_update = now
            witch_anim_index = (witch_anim_index + 1) % len(witch_frames_left)
        ...
        if witch_frames_left: # Check if the animation frames loaded
            ...
            SCREEN.blit(current_witch_img, witch_rect)
...
def main_menu():
    ...
    while True:
        ...
        if now - witch_anim_last_update > witch_anim_speed:
            witch_anim_last_update = now
            witch_anim_index = (witch_anim_index + 1) % len(witch_frames)

        active_frames = witch_frames_flipped if flip else witch_frames

        WITCH = Button(image=active_frames[witch_anim_index], pos=witch_pos, 
